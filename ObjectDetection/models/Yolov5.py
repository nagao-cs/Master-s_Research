...
class Yolov5nDetector(AbstractObjectDetector):
    ...
    def load_model(self):
        try:
            ...
            print(f"Error loading YOLOv5n model: {e}")
            self.model = None
    ...
    def save_result(self, image_path, bboxes, map, camera, index):
        ...
        # === 保存先のディレクトリを作成 ===
        output_image_dir = f"C:\CARLA_Latest\WindowsNoEditor\ObjectDetection/output/{map}/images/yolov5n_results/{camera}"
        output_label_dir = f"C:\CARLA_Latest\WindowsNoEditor\ObjectDetection/output/{map}/labels/yolov5n_results/{camera}"
        ...
        with open(output_label_path, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(['class_id', 'xmin', 'xmax',
                            'ymin', 'ymax', 'confidence'])
            for bbox in bboxes:
                writer.writerow([bbox['class_id'], bbox['xmin'], bbox['xmax'],
                                bbox['ymin'], bbox['ymax'], bbox['confidence']])
