...
if __name__ == "__main__":
    ...
    # 出力ディレクトリの整理
    # -----------
    outputLabelDir: Path = baseDir / "adaptiveDetectionResult" / f"{integrate_way}" / "labels" / \
    ...
    UNCERTAINTY_RATIO_THRESHOLD: float = 0.25

    print(
        f"信頼度の閾値:{CONF_THRESHOLD}, 一致の閾値:{AGREEMENT_THRESHOLD}")
    ...
    for frameIdx in tqdm(range(numFrames)):
        ...
        if executeState == VersionState.ONE:
            totalFLOPs += (FLOPs_Dict[baseModel])
        elif executeState == VersionState.N:
            totalFLOPs += (FLOPs_Dict[baseModel] +
