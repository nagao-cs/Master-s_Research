...
from ..config.config import SortConfig
...
class SortTracker:
    ...
    def update(self, detections: list[DetectionBoundingBox]) -> TrackingResult:
        ...
        for det in detections:
            by_label.setdefault(det.classId, []).append(det)
        ...
        for label, dets in by_label.items():
            ...
            tracks = sorter.update(dets_np)  # shape (K, 5): [x1, y1, x2, y2, track_id]
            ...
            for row in tracks:
                ...
                tracked_boxes.append(
                    DetectionBoundingBox(
                        ...
                        height=y2 - y1,
                        classId=label,
                        confidenceScore=1.0,  # SORT は confidence を保持しない
                    )
                )
    ...
    def _get_sorter(self, label: str) -> Sort:
        if label not in self._sorters:
            self._sorters[label] = Sort(
                max_age=self._cfg.max_age,
                min_hits=self._cfg.min_hits,
