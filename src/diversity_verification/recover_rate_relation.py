...
def isResolvedByThirdModel(
    ...
    if instance.kind == "FN" or instance.kind == "both FN":
        return matchedByThirdModel
    elif instance.kind == "FP" or instance.kind == "both FP":  # "FP"
...
class FrameRecoveryRecord:
    ...
    resolvedFp: int = 0
    totalBothFn: int = 0
    resolvedBothFn: int = 0
    totalBothFp: int = 0
    resolvedBothFp: int = 0
    resolvedBothModel: int = 0
    total_unresolved_both_model :int = 0
    ...
    @property
    def recoveryRateBothFn(self):
        return self.resolvedBothFn / self.totalBothFn if self.totalBothFn > 0 else float("nan")
    

    @property
    def recoveryRateBothFp(self):
        return self.resolvedBothFp / self.totalBothFp if self.totalBothFp > 0 else float("nan")
...
def processFrame(
    ...
    total_unresolved_one_model = 0
    total_unresolved_both_model = 0
    resolvedOneModel = 0
    resolvedBothModel = 0
    ...
    for inst in unresolvedInstances:
        ...
        elif inst.kind == "both FN":
            total_unresolved_both_model += 1
            totalBothFn += 1
            if resolved:
                resolvedBothFn += 1
                resolvedBothModel += 1

        elif inst.kind == "both FP":
            total_unresolved_both_model += 1
            totalBothFp += 1
            if resolved:
                resolvedBothFp += 1
    ...
    return FrameRecoveryRecord(
        imageId=imageId,
        jaccard=jaccard,
        totalUnresolved=total_unresolved_one_model,
        resolvedCount=resolvedOneModel,
        totalUnresolvedFn=totalUnresolvedFn,
        resolvedFn=resolvedFn,
        totalUnresolvedFp=totalUnresolvedFp,
        resolvedFp=resolvedFp,
        totalBothFn=totalBothFn,
        resolvedBothFn=resolvedBothFn,
        totalBothFp=totalBothFp,
        resolvedBothFp=resolvedBothFp,
        resolvedBothModel=resolvedBothModel,
        total_unresolved_both_model=total_unresolved_both_model
    )
...
def saveFrameRecordsToCsv(frameRecords: list[FrameRecoveryRecord], outputPath: Path) -> None:
    ...
    with open(outputPath, "w", newline="") as f:
        ...
        writer.writerow([
            ...
            "totalBothFn", "resolvedBothFn", "recoveryRateBothFn",
            "totalBothFp", "resolvedBothFp", "recoveryRateBothFp",
        ])
        for r in frameRecords:
            writer.writerow([
                ...
                r.totalBothFn, r.resolvedBothFn, r.recoveryRateBothFn,
                r.totalBothFp, r.resolvedBothFp, r.recoveryRateBothFp
            ])
...
if __name__ == "__main__":
    ...
    total_both_model_Fn = sum(record.totalBothFn for record in frameRecords)
    total_resolved_both_model_Fn = sum(record.resolvedBothFn for record in frameRecords)
    overallRecoveryRate_both_model_Fn = total_resolved_both_model_Fn / total_both_model_Fn if total_both_model_Fn > 0 else float("nan")

    total_both_model_Fp = sum(record.totalBothFp for record in frameRecords)
    total_resolved_both_model_Fp = sum(record.resolvedBothFp for record in frameRecords)
    overallRecoveryRate_both_model_Fp = total_resolved_both_model_Fp / total_both_model_Fp if total_both_model_Fp > 0 else float("nan")

    total_unresolved_both_model = sum(record.total_unresolved_both_model for record in frameRecords)
    total_resolved_both_model = sum(record.resolvedBothModel for record in frameRecords)
    overallRecoveryRate_both_model = total_resolved_both_model / total_resolved_both_model if total_resolved_both_model > 0 else float("nan")
    ...
    summary_fieldnames = [
        ...
        "total_both_model_fn", "total_resolved_both_model_fn", "overall_recovery_rate_both_fn",
        "total_both_model_fp", "total_resolved_both_model_fp", "overall_recovery_rate_both_fp",
        "total_unresolved_both_model", "total_resolved_both_model", "overall_recovery_rate_both_model",
    ...
    with open(summary_path, "a", newline="") as sf:
        ...
        writer.writerow({
            ...
            "total_both_model_fn": total_both_model_Fn,
            "total_resolved_both_model_fn": total_resolved_both_model_Fn,
            "overall_recovery_rate_both_fn": overallRecoveryRate_both_model_Fn,
            "total_both_model_fp": total_both_model_Fp, 
            "total_resolved_both_model_fp": total_resolved_both_model_Fp, 
            "overall_recovery_rate_both_fp": overallRecoveryRate_both_model_Fp,
            "total_unresolved_both_model": total_unresolved_both_model, 
            "total_resolved_both_model": total_resolved_both_model, 
            "overall_recovery_rate_both_model": overallRecoveryRate_both_model,
