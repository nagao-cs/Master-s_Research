...
class MobilenetDetector(Detector):
    ...
    def load_model(self):
        try:
            self.model = hub.load(
                "https://kaggle.com/models/google/mobilenet-v2/frameworks/TensorFlow1/variations/openimages-v4-ssd-mobilenet-v2/versions/1")
            print(f"Mobilenet model loaded")
    ...
    def save_result(self, image_path, bboxes, map, camera, index):
        ...
        original_width, original_height = image.shape[1], image.shape[0]
        # === 保存先のディレクトリを作成 ===
        output_image_dir = f"C:\CARLA_Latest\WindowsNoEditor\ObjectDetection\output/{map}/images/Mobilenet_results/{camera}"
        output_label_dir = f"C:\CARLA_Latest\WindowsNoEditor\ObjectDetection\output/{map}/labels/Mobilenet_results/{camera}"
        ...
        with open(output_label_path, 'w', newline='') as csvfile:
            fieldnames = ['class_id', 'xmin',
                          'xmax', 'ymin', 'ymax', 'confidence']
